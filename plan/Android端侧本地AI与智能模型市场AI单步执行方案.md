...
> **给 AI 执行者的强制规则：** 本文件不是一次性生成全部代码的提示词，而是项目执行控制协议。每次运行对话只能处理一个步骤 ID；完成该步骤后必须验证并追加 `docs/ai-work-log.md`，然后停止。不得在同一轮对话顺手执行下一步、重构无关代码或提前实现后续阶段。
...
一次 AI 对话只能有一个当前步骤，例如 `S007`。当前步骤必须来自第 4 节步骤表，并且依赖步骤已经在日志中标记为 `DONE`。如果无法满足依赖，AI 必须只记录 `BLOCKED`，不得自行跳到别的步骤。

每个步骤必须只产生一个可验收结果：一个文件、一个接口、一次构建、一次测试集或一个明确的配置变更。一个步骤中允许同时修改“实现文件 + 对应测试文件”，但不允许把多个产品能力合并成一个步骤。

### 1.2 AI 每次运行的固定顺序

1. 定位项目根目录，确认存在 `.git`。
2. 读取 `docs/ai-execution-plan.md` 和 `docs/ai-work-log.md` 的最新内容。
3. 执行 `git status --short`，识别已有未提交改动；不得覆盖不属于当前步骤的改动。
4. 从步骤表中确定唯一的 `STEP_ID`。若日志中已有 `IN_PROGRESS`，只能继续该 ID。
5. 在日志追加一条 `IN_PROGRESS`，写明计划修改的文件和验证命令。
6. 只完成该步骤卡中的动作，修改范围不得超出“允许修改”列表。
7. 执行该步骤卡中的验证命令；失败时修复当前步骤，仍不得开始下一步。
8. 在日志追加 `DONE` 或 `BLOCKED`，记录实际文件、命令输出摘要、风险和下一步依赖。
9. 向用户返回本轮结果，然后结束本轮对话。

### 1.3 严格禁止事项

- 不得一次对话完成两个或更多 `STEP_ID`。
- 不得因为发现顺手问题而修改步骤卡之外的文件；应写入 `BLOCKED` 或“后续风险”。
- 不得把“代码已写入”当作完成；没有验证命令结果只能标记 `IN_PROGRESS`。
- 不得删除或改写历史日志；日志只允许追加。
...
强制边界：本轮只执行一个步骤 <STEP_ID>，禁止执行任何其他步骤或无关重构。
项目根目录：<PROJECT_ROOT>
步骤文件：docs/ai-execution-plan.md
工作日志：docs/ai-work-log.md

开始前：
1. 读取步骤文件中 <STEP_ID> 的完整步骤卡。
2. 读取工作日志最后 120 行和最近一次状态。
3. 执行 git status --short，保护已有未提交改动。

执行中：只修改步骤卡允许的文件；遇到额外问题先停止并记录，不扩张范围。

结束前：执行步骤卡中的验证命令，并向 docs/ai-work-log.md 追加一条日志。验证未通过时只能写 IN_PROGRESS 或 BLOCKED，不能写 DONE。

回复格式必须是：
- STEP_ID / 状态：DONE、IN_PROGRESS 或 BLOCKED
- 本轮实际改动：文件路径 + 一句话
- 验证：命令 + 结果摘要
- 日志：已追加的日志标题
- 下一步：只写依赖关系，不执行下一步
...
```markdown
### 2026-08-22 09:30 | S001 | DONE

- 目标：用一句话说明本步骤要交付什么。
- 依赖：无，或列出已经 DONE 的步骤 ID。
- 实际修改：
  - `path/to/file`: 变更摘要。
- 验证：
  - 命令：`具体命令`
...
`DONE` 的含义是本步骤的验收标准全部通过，不代表整个项目完成。`BLOCKED` 必须包含阻塞原因、已尝试命令、需要的外部输入和解除条件。日志中的“下一步依赖”只用于导航，不表示 AI 已经执行该步骤。
...
每张步骤卡都包含：依赖、目标、允许修改的文件、唯一动作、验证命令、通过标准。AI 必须严格按卡执行。步骤卡中的路径以项目根目录为基准；不存在的目录由对应步骤创建。

若步骤卡要求“创建文件”，只能创建卡片列出的文件。若实现过程中发现需要新增文件，先将该步骤记为 `BLOCKED`，把新增文件理由写入日志，等待下一轮重新确认范围。
...
| 里程碑 | 通过条件 | 未通过时的动作 |
| --- | --- | --- |
| G0 工程基线 | 工具链、许可证清单、三台设备信息可复现 | 停在 S001-S004，禁止进入 Native |
| G1 Native 基线 | `arm64-v8a` CPU/NEON 完成加载、生成、停止、释放、重启 | 停在 S005-S012，记录设备和错误码 |
| G2 供应链 | Manifest 签名、断点下载、SHA-256、GGUF 解析、原子安装和回滚通过 | 停在 S013-S019 |
| G3 隔离与适配 | AIDL、独立进程、崩溃恢复、设备画像、内存规则和降级可解释 | 停在 S020-S028 |
| G4 产品闭环 | 市场、详情、下载管理、聊天、历史、诊断和设置可走通 | 停在 S029-S035 |
| G5 发布门禁 | 故障注入、三档设备性能、隐私/许可证、回滚和灰度通过 | 停在 S036-S041 |
...
- 验证：`git status --short`；确认两个文档存在，且日志包含“当前步骤”和“追加规则”。
...
- 依赖：S002 DONE。
- 目标：登记旗舰、中端、入门设备的 SoC、RAM、Android、ABI、页大小和可用存储。
- 允许修改：`qa/device-matrix/devices.yaml`、`docs/device-baseline.md`。
- 唯一动作：只填写设备基线，不运行推理 Benchmark。
- 验证：YAML 解析命令或 `python -c "import yaml; yaml.safe_load(open('qa/device-matrix/devices.yaml'))"`。
- 通过标准：至少三台真实设备、每台字段完整且无序列号等个人信息。
...
- 验证：设备命令或 Benchmark runner，记录加载时长、TTFT、输出 token 数、峰值 RSS/PSS。
- 通过标准：三档设备均可加载并生成，失败有结构化错误码。
...
- 验证：Native 单元测试或真机 runner，确认顺序、结束事件和异常事件。
...
#### S010 | 建立三档设备短 Benchmark

- 依赖：S009 DONE。
- 目标：固定输入、输出上限、线程数和采样参数，输出 TTFT、TPS、峰值内存、温度和电量。
- 允许修改：`app/src/main/cpp/benchmark/`、`qa/benchmarks/`、`docs/benchmark-protocol.md`。
- 唯一动作：只建立可重复 Benchmark，不做推荐算法。
- 验证：在三档设备各运行 3 次，保存 JSON 结果。
- 通过标准：结果字段一致，P50/P95 可计算，异常样本保留原因。

#### S011 | 完成 G1 门禁复核

- 依赖：S007、S008、S009、S010 DONE。
- 目标：汇总 Native 基线的构建、加载、生成、停止、释放和重启证据。
- 允许修改：`docs/gates/g1-native.md`。
- 唯一动作：只写门禁报告，不修改实现代码。
- 验证：逐项引用 S005-S010 日志和产物路径。
- 通过标准：G1 报告明确 PASS 或 BLOCKED；未 PASS 不得开始 S012 之后的供应链任务。
...
- 验证：三档真机字段完整性测试，确认不记录序列号。
- 通过标准：画像字段有来源、时间和“未知”处理。
...
- 唯一动作：只写门禁报告。
- 验证：引用对应日志、崩溃测试和三档设备结果。
- 通过标准：G3 PASS 后才允许接入完整 UI。
...
- 唯一动作：只执行端到端复核并写报告。
- 验证：真机端到端测试，保留截图、日志摘要和失败步骤。
- 通过标准：主流程可完成，任意失败都有可见原因和恢复入口。
...
- 依赖：S010、S035 DONE。
- 目标：固定模型、输入、输出上限、温度、电量和采样条件，比较基线回归。
- 允许修改：`qa/benchmarks/`、`docs/performance-regression.md`。
- 唯一动作：只跑性能回归，不调参数追逐单一峰值。
- 验证：每档设备至少 3 次，记录 P50/P95、温度、电量和峰值 RSS/PSS。
- 通过标准：无超过门禁的回归；异常值标注原因而非删除。
