...
前端第一阶段采用“Material 3 组件基线 + 本地 AI 产品信息架构 + 自定义视觉主题”的组合：

| 参考 | 用途 | 本项目采用方式 |
| --- | --- | --- |
| [Google AI Edge Gallery](https://github.com/google-ai-edge/gallery) | 模型发现、模型管理、体验入口和 Benchmark 信息架构 | 借鉴页面层级、模型卡片信息密度和体验入口；不复制 Kotlin/LiteRT 实现或 Google 品牌资产；Apache-2.0 许可证需保留 NOTICE |
| [PocketPal AI](https://github.com/a-ghorbani/pocketpal-ai) | 本地聊天、模型切换、离线隐私和性能展示 | 借鉴聊天流程、模型管理和诊断表达；不复制 React Native 代码或品牌；MIT 许可证仍需保留版权声明 |
| [SmolChat-Android](https://github.com/shubham0204/SmolChat-Android) | Android GGUF/JNI 最小工程和聊天交互 | 只参考工程边界和交互问题；Apache-2.0 代码必须单独审查，不直接搬运 UI |
| [Material 3](https://m3.material.io/) | 色彩、排版、组件状态、无障碍和响应式布局 | 使用 Android 原生 Material 组件作为可访问性基线，自定义品牌色、插图和信息密度 |

前端美观度是 P1 的硬性验收项：必须有设计令牌、统一字体层级、8dp 间距基线、明确的主次色、完整的加载/空态/错误态/深色模式、稳定的小屏布局和可读的中文文案；不得保留默认按钮、默认列表、无层级的白底堆叠或被截断的文本。验收至少覆盖市场首页、模型详情、聊天页和下载管理页四个核心状态。
