...
图片文字识别使用 ONNX Runtime Android 1.24.3（MIT 及其内附第三方声明）和 PaddleOCR PP-OCRv4 中文移动端模型（Apache-2.0）。模型来自 RapidAI 的 ModelScope v3.9.2 固定版本，随 APK 提供，来源与 SHA-256 记录在 `assets/ocr/models.json`，构建时验证。模型转换方 RapidOCR 的 Apache-2.0 声明一并保留。运行期只使用本地 CPU，无云识别接口、Google Play 服务或 ML Kit。
