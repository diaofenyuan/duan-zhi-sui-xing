...
public final class ReplyComposer {
    ...
    /** 将回复切成小片段，模拟 token 流。 */
    public static String[] chunk(String reply) {
        String[] parts = new String[(reply.length() + 1) / 2];
        for (int i = 0, j = 0; i < reply.length(); i += 2, j++) {
            parts[j] = reply.substring(i, Math.min(reply.length(), i + 2));
        }
        return parts;
    }
}
