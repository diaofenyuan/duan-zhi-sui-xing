...
public class InferenceServiceInstrumentedTest {
    ...
    private static final class Recorder implements InferenceClient.Events {
        ...
        final AtomicInteger finishReason = new AtomicInteger(-1);
        ...
        boolean awaitFinished() throws InterruptedException {
            return finished.await(TIMEOUT_SEC, TimeUnit.SECONDS);
        }
    }

    private InferenceRequest requestFor(String path) {
        return new InferenceRequest("test", "test-model", "test",
     ...
     */
    private static String chatml(String user) {
        return "<|im_start|>user\n" + user + "<|im_end|>\n<|im_start|>assistant\n";
    }

    private InferenceRequest approvedRequest() {
        return ApprovedModels.requestFor(context, ApprovedModels.SMOLLM_135M, "test-approved");
    }

    /** 限定输出上限的批准模型请求：模拟器性能波动下仍能在时限内结束。 */
    private InferenceRequest smallApprovedRequest(int maxNewTokens) {
        ApprovedModels.Approved a = ApprovedModels.SMOLLM_135M;
        return new InferenceRequest("test-approved", a.modelId, a.version,
                ApprovedModels.modelFile(context, a).getAbsolutePath(),
                a.contextLength, a.threadCount, a.temperature, a.topP, maxNewTokens);
    }

    private void assumeApprovedInstalled() {
    ...
    private File writeGarbageModel() throws Exception {
        File dir = new File(context.getFilesDir(), "models/bad/1");
        assertTrue(dir.exists() || dir.mkdirs());
        File file = new File(dir, "bad.gguf");
    ...
    private int readServicePid() throws Exception {
        final int[] pidHolder = {-1};
        final CountDownLatch latch = new CountDownLatch(1);
        boolean ok = context.bindService(
                new Intent(context, InferenceService.class),
    ...
    @Test
    public void invalidModelFile_returnsModelLoadFailedCode() throws Exception {
        File junk = writeGarbageModel();
        client = new InferenceClient(context);
        Recorder recorder = new Recorder();
        client.connect(requestFor(junk.getAbsolutePath()), recorder);
    ...
    @Test
    public void pathOutsideFilesDir_rejectedWithIllegalArgument() throws Exception {
        client = new InferenceClient(context);
        Recorder recorder = new Recorder();
    ...
    @Test
    public void serviceRunsInSeparateProcess() throws Exception {
        ...
        int servicePid = readServicePid();
        assertTrue("service pid not found", servicePid > 0);
        assertNotEquals(Process.myPid(), servicePid);
    }

    @Test
    public void realModel_generateThenStopMidStream_thenRepeatedStopRelease() throws Exception {
        assumeApprovedInstalled();
        client = new InferenceClient(context);
    ...
    @Test
    public void realModel_serviceKill_engineCrashed_thenRestartRecovers() throws Exception {
        assumeApprovedInstalled();
        client = new InferenceClient(context);
        Recorder first = new Recorder();
        client.connect(smallApprovedRequest(48), first);
        ...
        Recorder recovery = new Recorder();
    ...
    @Test
    public void realModel_repeatedGenerateCycles_doNotCrash() throws Exception {
        assumeApprovedInstalled();
        client = new InferenceClient(context);
        Recorder recorder = new Recorder();
        client.connect(smallApprovedRequest(64), recorder);
